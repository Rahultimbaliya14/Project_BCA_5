...
<head>
    ...
    <link rel="stylesheet" href="../Css/ofprofile.css">
...
<body>
    ...
    if(isset($_GET['first'])){
        $id=$_GET['id'];
        $query="SELECT * FROM officialuser3 WHERE id=$id";
        $result=mysqli_query($con,$query);
         $row=mysqli_fetch_assoc($result);
         if($_GET['first']==2){
            echo "<script>alert('Profile Updated Successfully')</script>";
            // header("Location: ../View/adminprofile.php?first=1&id=$id&mass=2");
            echo "<script>window.location.replace('http://localhost:7878/project/Views/OfProfile.php?first=1&id=$id');</script>";
          }
    ...
    else
    {
            ...
            $row=mysqli_fetch_assoc($result);
        ?>
        <form action="../Src/ofupdate.php" method="post" enctype="multipart/form-data"> 
        <div class="profile">
